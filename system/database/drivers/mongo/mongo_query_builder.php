        ...
        if(!empty($this->qb_groupby)) {
            if(!empty($this->qb_select)) {
                foreach($this->qb_select as $select_key=>$select_val) {
                    if($select_val){
                        // $this->qb_groupby['$group']['_id'][$select_key] = '$'.$select_key;
                        // $this->qb_groupby['$group'][$select_key]['$addToSet'] = '$'.$select_key;
                        if(!isset($this->qb_groupby['$group'][$select_key])) {
                            $this->qb_groupby['$group']['detail']['$push'][$select_key] = '$'.$select_key ;
                        }
                    }
                }
            }
            if(!empty($this->qb_orderby)) {
                foreach($this->qb_orderby as $select_key=>$select_val) {
                    if($select_val){
                        $this->qb_groupby['$group'][$select_key]['$addToSet'] = '$'.$select_key;
                    }
                }
...
//        $qb_cache_key = ($qb_key === 'qb_having') ? 'qb_cache_having' : 'qb_cache_where';
    ...
    {
        if(is_string($orderby) && strpos(trim($orderby), ',')) {
